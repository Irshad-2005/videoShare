...
const addComment = asyncHandlers(async (req, res) => {
      ...
      res.status(201).json(
            new ApiResponse(201, comment, "comment are successfully create")
      );
});
...
const updateComment = asyncHandlers(async (req, res) => {
      ...
      res.status(200).json(
            new ApiResponse(200, comment, "comment are update successfully")
      );
});
...
const deleteComment = asyncHandlers(async (req, res) => {
      ...
      res.status(200).json(
            new ApiResponse(200, comment, "comment are delete successfully")
      );
});
