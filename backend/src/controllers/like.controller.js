...
const toggleVideoLike = asyncHandlers(async (req, res) => {
        ...
        if (!isValidObjectId(videoId)) {
                return res
                        .status(400)
                        .json(new ApiError(200, "videoId are not valid"));
        }
        ...
        return res
                .status(201)
                .json(new ApiResponse(201, newLike, "like to video "));
});
...
const toggleCommentLike = asyncHandlers(async (req, res) => {
        ...
        if (!isValidObjectId(commentId)) {
                return res
                        .status(400)
                        .json(new ApiError(400, "commentId are not valid"));
        }
...
const toggleTweetLike = asyncHandlers(async (req, res) => {
        ...
        if (tweetLikePresent) {
                const removeTweetLike = await Like.findByIdAndDelete(
                        tweetLikePresent._id
                );
                return res
                        .status(200)
                        .json(
        ...
        res.status(201).json(
                new ApiResponse(201, tweetLike, "tweet are likes successfully")
        );
});
